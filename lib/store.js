  ...
  await db.transaction(async (client) => {
    ...
    await client.query(
      `INSERT INTO players (id, number, username, display_name, position, is_admin, sort_order, claimed, password_salt, password_hash)
       VALUES ($1,$2,$3,$4,$5,true,-1,true,$6,$7)`,
    ...
    for (const entry of SEED_PLAYERS) {
      ...
      await client.query(
        `INSERT INTO players (id, number, username, display_name, position, is_admin, sort_order, claimed, password_salt, password_hash)
         VALUES ($1,$2,$3,$4,$5,false,$6,false,$7,$8)`,
        [
...
async function listRoster() {
  ...
  return players.filter((p) => !p.isAdmin).sort(playerSort);
}
...
async function pendingPlayers() {
  const { rows } = await db.query(
    'SELECT * FROM players WHERE NOT is_admin AND NOT claimed ORDER BY sort_order, username',
  );
...
async function claimAccount(username, password) {
  ...
  if (player.isAdmin) throw new HttpError(403, 'La cuenta de administrador no se registra desde aquí');
...
async function resetAccount(playerId) {
  ...
  if (player.isAdmin) throw new HttpError(400, 'La cuenta de administrador no se restablece desde aquí');
