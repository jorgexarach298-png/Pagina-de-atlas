 ...
 * Ayudas y pruebas
 *
 * Gente de fuera que completa el once cuando falta alguien. No son miembros:
 * no salen en la Plantilla, no firman check-in, no reciben notas y no pueden
 * iniciar sesión. Pero sí ocupan una ficha en el campo, así que viven en
 * `players` con is_guest = true: es lo que mantiene en pie las claves ajenas
 * del once, de las estadísticas y de las papeletas.
 * ------------------------------------------------------------------ */

/** Tope de ayudas distintas: evita que la lista crezca sin control. */
const MAX_GUESTS = 30;

/** Posición por defecto de una ayuda: sin dorsal ni puesto concreto. */
const GUEST_POSITION = 'DC';

function normaliseGuestName(name) {
  // Se colapsan los espacios para que «Juan  Pérez» y «Juan Pérez» sean la
  // misma ayuda y no se dupliquen en la lista.
  return String(name || '').replace(/\s+/g, ' ').trim().slice(0, 40);
}

async function findGuestByName(name) {
  const clean = normaliseGuestName(name);
  if (!clean) return null;
  const { rows } = await db.query(
    'SELECT * FROM players WHERE is_guest AND lower(display_name) = lower($1)',
    [clean],
  );
  return playerFromRow(rows[0]);
}
...
async function listGuests() {
  const { rows } = await db.query('SELECT * FROM players WHERE is_guest ORDER BY sort_order, display_name');
  return rows.map(playerFromRow);
}

/**
 * Devuelve la ayuda con ese nombre y, si no existe, la crea. Repetir el nombre
 * reutiliza la misma ficha: así no se llena la lista de duplicados.
 */
async function ensureGuest(name) {
  const clean = normaliseGuestName(name);
  if (!clean) throw new HttpError(400, 'Ponle un nombre a la ayuda');

  const existing = await findGuestByName(clean);
  if (existing) return { guest: publicPlayer(existing), created: false };

  const { rows: count } = await db.query('SELECT count(*)::int AS n FROM players WHERE is_guest');
  if (count[0].n >= MAX_GUESTS) {
    throw new HttpError(400, `Ya hay ${MAX_GUESTS} ayudas guardadas. Borra alguna antes de añadir más.`);
  }

  // Sin cuenta: la contraseña es un secreto aleatorio que nadie conoce, y así
  // estas fichas no pueden iniciar sesión ni activarse desde «Registrarme».
  const secret = hashPassword(crypto.randomBytes(24).toString('hex'));
  const { rows } = await db.query(
    `INSERT INTO players (id, number, username, display_name, position, is_admin, is_player, is_guest, sort_order, claimed, password_salt, password_hash)
     VALUES ($1, '', $2, $3, $4, false, false, true,
             (SELECT COALESCE(MAX(sort_order), -1) + 1 FROM players), true, $5, $6)
     RETURNING *`,
    [guestId(clean), clean, clean, GUEST_POSITION, secret.salt, secret.hash],
  );
  return { guest: publicPlayer(playerFromRow(rows[0])), created: true };
}

/**
 * ID interno de una ayuda. No se muestra en la web (la ficha se identifica por
 * el nombre), así que basta con que sea único y no choque con un ID de plantilla.
 */
function guestId(name) {
  return `ayuda-${slugId(name)}-${crypto.randomUUID().slice(0, 6)}`;
}

async function deleteGuest(id) {
  const guest = await findPlayerById(id);
  if (!guest || !guest.isGuest) return false;
  // Sus fichas, estadísticas y papeletas caen en cascada con las claves ajenas.
  await db.query('DELETE FROM players WHERE id = $1', [id]);
  return true;
}

/**
 * Cambia el nombre de una ayuda. Se conservan su ID y sus fichas, así que
 * renombrarla no la saca del once ni pierde lo ya anotado.
 */
async function renameGuest(id, name) {
  const guest = await findPlayerById(id);
  if (!guest || !guest.isGuest) throw new HttpError(404, 'Esa ayuda no existe');

  const clean = normaliseGuestName(name);
  if (!clean) throw new HttpError(400, 'Ponle un nombre a la ayuda');

  const clash = await findGuestByName(clean);
  if (clash && clash.id !== id) throw new HttpError(409, `Ya hay una ayuda llamada «${clean}»`);

  const { rows } = await db.query(
    'UPDATE players SET display_name = $2, username = $2, updated_at = now() WHERE id = $1 RETURNING *',
    [id, clean],
  );
  return publicPlayer(playerFromRow(rows[0]));
}

/** Foto de una ayuda, para que su ficha no salga solo con las iniciales. */
async function setGuestPhoto(id, photo) {
  const guest = await findPlayerById(id);
  if (!guest || !guest.isGuest) throw new HttpError(404, 'Esa ayuda no existe');

  const { rows } = await db.query(
    'UPDATE players SET photo = $2, updated_at = now() WHERE id = $1 RETURNING *',
    [id, photo || null],
  );
  return publicPlayer(playerFromRow(rows[0]));
}
...
async function authenticate(username, password) {
  ...
  // Las ayudas no tienen cuenta: aunque su contraseña es un secreto aleatorio,
  // se rechazan explícitamente para que no puedan entrar ni por error.
  if (!player || player.isGuest || !verifyPassword(password, player.credentials)) return null;
...
async function publishMatch(date, { formation, items, note }) {
  // El once admite tanto miembros de la plantilla como ayudas de fuera: lo que
  // se publica es lo que hay sobre el campo.
  const eligible = new Set([...(await listRoster()).map((p) => p.id), ...(await listGuests()).map((p) => p.id)]);
  const cleanItems = (items || [])
    .filter((item) => item && eligible.has(item.playerId))
    .map((item) => ({
...
async function saveBallot(date, voterId, scores) {
  ...
  const clean = {};
  const used = new Map();
  // Las ayudas juegan, pero no son compañeros a los que puntuar: no tienen
  // carta ni media, así que no se les puede poner nota.
  const guests = new Set((await listGuests()).map((g) => g.id));
  for (const [targetId, raw] of Object.entries(scores || {})) {
    ...
    if (guests.has(targetId)) continue;
